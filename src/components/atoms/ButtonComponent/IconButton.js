...
function Icons({label}) {
  ...
  if (label === 'filter') {
    return (
      <Icon name="filter-list" size={24} color={colors.background.black} />
    );
  }
  return (
    <Icon name="fingerprint" size={25} color={colors.background.secondary} />
  );
}
function IconButton({onPress, nonButton, label, style}) {
  return (
    <View>
      {nonButton ? (
